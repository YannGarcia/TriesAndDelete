...
REFERENCES="LibCommon LibHttp LibPemea"
for i in ${REFERENCES}
do
    # TTCN code
    for j in `find ${PATH_DEV_EMTEL}/src/$i/ttcn -type f -name "*.ttcn"`;
    do
	      ln -sf $j ../ttcn/`basename $j`
    done
    # Include source code
    files=`find ${PATH_DEV_EMTEL}/src/$i/include -type f`
    if [ "${files}" != " " ]
    then
	      for j in ${files};
	      do
    ...
    # CC source code
    files=`find ${PATH_DEV_EMTEL}/src/$i/src -type f`
    if [ "${files}" != " " ]
    then
	      for j in ${files};
	      do
...
then
    rm ../bin/*.exe ../lib/*.dll
    compiler.exe -b -d -e -f -g -j -l -L -n -O -t -R -U none -x -X ${TTCN_FILES} 2>&1 3>&1 | tee build.log
    if [ "$?" == "1" ]
    then
        f_exit "Failed to compile ATS" 4
    fi
else
    compiler -b -d -e -f -g -j -l -L -n -O -t -R -U none -x -X ${TTCN_FILES} 2>&1 3>&1 | tee build.log
    if [ "$?" == "1" ]
    then 
...
FWK_FILES=`find ${PATH_DEV_EMTEL}/framework/ -name '*.c*'`
